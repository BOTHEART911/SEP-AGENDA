 ...
 *
 * 12/08/2026: antes de ver el contrato hay una PUERTA con las cinco
 * casillas del ACUERDO DE USO DE MECANISMO DE FIRMA ELECTRÓNICA (se
 * piden cada vez que entra); desaparecen las dos declaraciones que
 * estaban bajo las firmas, y se mantiene la aceptación de los términos
...
var CONTRATO = (function () {
  ...
  var S = {
    ...
    form: null,
    pngs: {},
    /* 12/08/2026 — las 5 casillas del Acuerdo de firma electrónica. Es la
       PUERTA: se piden antes de ver una sola línea del contrato y se vuelven
       a pedir cada vez que el estudiante entra (por eso se reinician en
       abrir() y no se guardan en el dispositivo). */
    acuerdo: []
  };

  /* Texto literal del Acuerdo. Cada punto es una casilla y las cinco son
     obligatorias; el backend vuelve a exigirlas al firmar. */
  var ACUERDO_TITULO = 'ACUERDO DE USO DE MECANISMO DE FIRMA ELECTRÓNICA Y ACEPTACIÓN DE CONDICIONES';

  var ACUERDO = [
    { t: 'Objeto y Aceptación del Mecanismo',
      x: 'De conformidad con lo establecido en el Decreto 1074 de 2015, mediante el presente acuerdo de voluntades las partes estipulan las condiciones legales y técnicas a las cuales se ajustarán para crear el presente documento electrónico y manifestar su consentimiento. EL USUARIO (Participante y/o Deudor Solidario) acepta expresamente que el uso de su clave de acceso personal y contraseña dentro del portal de SEP COLOMBIA constituye un método de firma electrónica válido.' },
    { t: 'Validez, Equivalencia Funcional y Confiabilidad',
      x: 'Las partes reconocen y aceptan que este mecanismo de firma electrónica permite identificar de manera inequívoca al firmante en relación con el mensaje de datos y sirve para indicar que el contenido del contrato cuenta con su total aprobación. En consecuencia, ambas partes presumen y aceptan que este método es técnicamente seguro, confiable y apropiado para los fines previstos, otorgándole la misma validez y plenos efectos jurídicos que una firma autógrafa o manuscrita.' },
    { t: 'Obligaciones y Custodia del Firmante',
      x: 'Al ser la clave de acceso un dato único y personalísimo, EL USUARIO se obliga incondicionalmente a: a) Mantener estricto control y custodia sobre sus datos de creación de firma (usuario y contraseña); b) Actuar con la máxima diligencia para evitar la utilización no autorizada de sus credenciales por parte de terceros; c) Dar aviso de manera inmediata a SEP COLOMBIA si tiene conocimiento de que sus datos de acceso han quedado en entredicho, se han conocido ilegalmente por terceros, o existe un riesgo considerable de que hayan perdido su confidencialidad.' },
    { t: 'Consecuencias por Fraude o Suplantación',
      x: 'Se advierte expresamente que el uso de las credenciales de acceso es personal e intransferible. En ningún caso el ESTUDIANTE PARTICIPANTE podrá utilizar la clave del DEUDOR SOLIDARIO para firmar en su nombre, representación o sustitución. En el evento de comprobarse que existió suplantación en la firma electrónica, el responsable asumirá de manera exclusiva y directa todas las consecuencias legales, civiles y penales derivadas de este hecho, incurriendo potencialmente en los delitos de Falsedad en documento privado y Falsedad personal por suplantación, los cuales conllevan penas de prisión.' },
    { t: '',
      x: 'He leído, entiendo y acepto las condiciones de este Acuerdo sobre el uso del mecanismo de firma electrónica.' }
  ];
  ...
  async function abrir() {
    ...
    try {
      ...
      S.acuerdo = ACUERDO.map(function () { return false; });
  ...
  /* ============================================================
     ACUERDO DE FIRMA ELECTRÓNICA — PUERTA (12/08/2026)
     ============================================================
     Primera pantalla del contrato: hasta que las cinco casillas no
     estén marcadas, el estudiante no ve el texto del contrato. Se pide
     cada vez que entra; nada de esto queda guardado en el dispositivo. */
  function acuerdoItemHtml(it, i) {
    return '' +
      '<label class="ctr-acu-i" id="ctr-acu-l-' + i + '">' +
      '  <input type="checkbox" class="ctr-acu-c" data-i="' + i + '"' + (S.acuerdo[i] ? ' checked' : '') + '>' +
      '  <span class="ctr-acu-x">' +
      (it.t ? '<b>' + esc(it.t) + ':</b> ' : '') + esc(it.x) +
      '  </span>' +
      '</label>';
  }

  function acuerdoContador() {
    var n = S.acuerdo.filter(Boolean).length;
    var el = q('#ctr-acu-n');
    if (el) el.textContent = n + ' de ' + ACUERDO.length + ' aceptadas';
  }

  function pintarAcuerdo() {
    q('#ctr-sub').textContent = 'Antes de leer tu contrato';
    q('#ctr-bar').style.width = '4%';

    var cont = q('#ctr-cont');
    cont.innerHTML = '' +
      '<div class="card ctr-acu">' +
      '  <h2><span class="em">🔐</span> ' + esc(ACUERDO_TITULO) + '</h2>' +
      '  <p class="muted">Para continuar debes leer y aceptar los cinco puntos. Son obligatorios.</p>' +
      ACUERDO.map(acuerdoItemHtml).join('') +
      '  <p class="muted center ctr-acu-n" id="ctr-acu-n">0 de ' + ACUERDO.length + ' aceptadas</p>' +
      '</div>' +
      '<div class="ctr-nav">' +
      '  <button class="btn btn-ghost" id="ctr-acu-salir">← Salir</button>' +
      '  <button class="btn btn-accent" id="ctr-acu-sig">Continuar a mi contrato →</button>' +
      '</div>';
    ...
    Array.prototype.forEach.call(cont.querySelectorAll('.ctr-acu-c'), function (c) {
      c.addEventListener('change', function () {
        S.acuerdo[Number(c.dataset.i)] = !!c.checked;
        acuerdoContador();
      });
    });

    q('#ctr-acu-salir').addEventListener('click', volverInicio);
    q('#ctr-acu-sig').addEventListener('click', function () {
      var falta = S.acuerdo.indexOf(false);
      if (falta < 0 && S.acuerdo.length === ACUERDO.length) { pintarLectura(); return; }
      var caja = q('#ctr-acu-l-' + (falta < 0 ? 0 : falta));
      if (caja) {
        caja.classList.add('ctr-falta');
        caja.scrollIntoView({ behavior: 'smooth', block: 'center' });
        setTimeout(function () { caja.classList.remove('ctr-falta'); }, 1400);
      }
      aviso('Para continuar debes aceptar los cinco puntos del Acuerdo.');
    });
  }
  ...
  function pintarFormulario() {
    ...
    q('#ctr-cont').innerHTML = '' +
      ...
      /* LOTE 17/08 (ajuste 2) — ya no hay calendario del navegador: la
         fecha se elige en la MISMA rueda manejada que usa el
         formulario. El valor sigue viajando en ISO (aaaa-mm-dd) dentro
         de un campo oculto, así que ni leerForm ni la validación ni el
         backend cambian. */
      '  <div class="ctr-f"><label>Fecha de nacimiento</label>' +
      '    <input id="f-nac" type="hidden" value="' + esc(d.nacimiento || '') + '">' +
      '    <button type="button" class="fm-fecha' + (d.nacimiento ? ' con' : '') + '" id="f-nac-btn">' +
      '      <span class="fm-fecha-ic">📅</span>' +
      '      <span class="fm-fecha-t" id="f-nac-t">' +
             (d.nacimiento ? esc(fechaTexto_(d.nacimiento)) : 'Elige la fecha') + '</span>' +
      '    </button></div>' +
    ...
    cablearArchivos();
    q('#ctr-volver-lect').addEventListener('click', function () { S.form = leerForm(); guardarFirmas(); pintarLectura(); });
  ...
  /* LOTE 17/08 — la rueda trabaja en dd/mm/aaaa y el contrato guarda
     ISO. Estas dos pasan de un formato al otro sin inventar fechas: si
     lo que llega no tiene la forma esperada, se devuelve vacío. */
  function isoADmy_(iso) {
    var p = String(iso || '').split('-');
    if (p.length !== 3 || p[0].length !== 4) return '';
    return p[2] + '/' + p[1] + '/' + p[0];
  }
  function dmyAIso_(dmy) {
    var m = /^(\d{2})\/(\d{2})\/(\d{4})$/.exec(String(dmy || ''));
    return m ? (m[3] + '-' + m[2] + '-' + m[1]) : '';
  }

  /* El botón abre la rueda del formulario (FORMU.rueda) acotada a los
     años del programa: de 17 a 28. El tope EXACTO por día lo sigue
     comprobando irAResumen con limiteNac_, que no se tocó. */
  function cablearNacimiento_() {
    var btn = q('#f-nac-btn');
    if (!btn) return;
    if (typeof FORMU === 'undefined' || !FORMU || typeof FORMU.rueda !== 'function') {
      /* Sin la rueda no se deja al estudiante sin poder escribir su
         fecha: el campo vuelve a ser uno de texto con su formato. */
      var oculto = q('#f-nac');
      oculto.type = 'text';
      oculto.placeholder = 'aaaa-mm-dd';
      btn.parentNode.removeChild(btn);
      return;
    }
    var hoy = new Date();
    btn.addEventListener('click', function () {
      FORMU.rueda(isoADmy_(q('#f-nac').value), { min: hoy.getFullYear() - 28, max: hoy.getFullYear() - 17 },
        'Tu fecha de nacimiento', function (dmy) {
          var iso = dmyAIso_(dmy);
          if (!iso) return;
          q('#f-nac').value = iso;
          q('#f-nac-t').textContent = fechaTexto_(iso);
  ...
  async function enviar() {
    ...
    try {
      var res = await apiPost('firmarContrato', Object.assign(cred_(), {
        ...
        firmaEst: S.pngs.est, firmaDeu: S.pngs.deu, acepta: true, acuerdo: true
      }));
      await creandoListo();
      pintarListo(res.url);
  ...
  /* LOTE 17/08 — el aviso del avión se comparte con el formulario. */
  return {
    tarjeta: tarjeta, bind: bind, abrir: abrir,
    cargando: { abrir: abrirCreando, listo: creandoListo, cerrar: cerrarCreando }
  };
})();
