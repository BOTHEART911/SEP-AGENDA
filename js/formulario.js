...
var FORMU = (function () {
  ...
  function tarjeta(e) {
    ...
    if (/^Reabierto/i.test(txt(f.resumen))) {
      return '' +
        '<div class="card">' +
        '  <h2><span class="em">📝</span> Tu formulario</h2>' +
        '  <div class="fm-reab-aviso">' +
        '    <div class="fm-reab-ic">↩️</div>' +
        '    <p><b>Tienes bloques por corregir.</b> Revisamos tu formulario y necesitamos que ajustes algo. ' +
        'Ábrelos, corrige lo que te pedimos y vuelve a enviarlos: lo demás sigue guardado como estaba.</p>' +
        '  </div>' +
        '  <div class="fm-prog"><i style="width:' + pct + '%"></i></div>' +
        '  <p class="muted center fm-prog-t">' + esc(f.resumen || '') + '</p>' +
        '  <button class="btn btn-accent btn-block fm-btn-reab" id="form-abrir">↩️ Corregir mis bloques</button>' +
        video +
        '</div>';
    }
  ...
  function pillEstado_(b) {
    ...
    if (reabierto_(b)) return '<span class="fm-pill reab">Por corregir ↩️</span>';
  ...
  function pintarLista() {
    ...
    var items = d.bloques.map(function (b) {
      ...
      var sub;
      if (reab) {
        sub = 'Te pedimos corregir este bloque';
      } else if (b.estadoVista === 'COMPLETADO') {
        sub = 'Guardado el ' + esc(b.fecha || '—') + (b.editableSiempre ? ' · puedes actualizarlo' : '');
      } else {
        sub = abierto ? (b.opcional ? 'Puedes dejarlo pendiente' : 'Te toca ahora')
                      : 'Se abre cuando completes el anterior';
      }
      return '' +
  ...
  function avisoReaperturaHtml_() {
    ...
    if (!motivos.length) {
      ...
      cuerpo = '<p class="fm-reab-m">Abre ' + (uno ? 'el bloque marcado' : 'los bloques marcados') +
               ' y revisa la información antes de volver a enviarlo' + (uno ? '' : 's') + '.</p>';
    } else if (motivos.length === 1) {
      cuerpo = '<p class="fm-reab-m">«' + esc(motivos[0]) + '»</p>';
    } else {
      cuerpo = '<ul class="fm-reab-l">' + lista.map(function (b) {
        var m = motivoReab_(b);
        return '<li><b>Bloque ' + b.n + ' · ' + esc(b.titulo) + ':</b> ' +
               (m ? '«' + esc(m) + '»' : 'revisa la información de este bloque.') + '</li>';
      }).join('') + '</ul>';
    }
    ...
    return '' +
      '<div class="fm-reab-aviso">' +
      '  <div class="fm-reab-ic">↩️</div>' +
      '  <h3>' + (uno ? 'Tienes 1 bloque por corregir' : 'Tienes ' + lista.length + ' bloques por corregir') + '</h3>' +
      '  <p>Revisamos tu formulario y necesitamos que ajustes esto. Los demás bloques siguen guardados como estaban.</p>' +
      cuerpo +
      '</div>';
  }
  ...
  function notaReaperturaHtml_(b) {
    ...
    return '' +
      ...
      '  <h3>↩️ Te pedimos corregir este bloque</h3>' +
      (m ? '  <p class="fm-reab-m">«' + esc(m) + '»</p>'
         : '  <p class="fm-reab-m">Revisa la información que guardaste antes de volver a enviarla.</p>') +
      '  <p class="fm-reab-p">Corrige lo que haga falta y envíalo otra vez. Lo demás que ya guardaste no se pierde.</p>' +
      '</div>';
  }
  ...
  function pintarBloque(conservarScroll) {
    ...
    if (soloDocs) {
      ...
      pie = '<div class="ctr-nav">' +
            ...
            (reabierto_(b) ? 'Guardar la corrección'
  ...
  async function guardarPaso_(pendiente) {
    ...
    try {
      ...
      if (carga) {
        await carga.listo({
          titulo: pendiente ? 'Listo, quedó pendiente' : '¡Listo! Tu bloque quedó guardado',
          sub: pendiente
  ...
  function rueda(valor, modo, titulo, onOk) {
    cablearRueda_();
    abrirRueda_(valor, modo, titulo, onOk);
  }
