...
(function () {
  ...
  function tarjeta(e) {
    ...
    if (o.mia) {
      return '' +
        ...
        '    </div>' +
        '  </div>' +
        '  <p class="muted center" style="margin:10px 0 0">Seleccionada el ' + esc(o.mia.fecha) +
  ...
  function pintarLista() {
    ...
    if (d.mia) {
      html += '' +
        ...
        '    </div>' +
        '  </div>' +
        '  <p class="muted">Sponsor <b>' + esc(d.mia.sponsor) + '</b> · seleccionada el ' +
  ...
  function pintarFicha() {
    ...
    if (S.ficha.mia) {
      acciones = '<div class="card ofe-lock-card"><div class="ofe-lock">' +
        '<div class="ofe-lock-ic">📌</div><p>Ya tienes una oferta seleccionada: <b>' +
        esc(S.ficha.mia.posicion) + '</b> en ' + esc(S.ficha.mia.empleador) + '.</p></div></div>';
    ...
    if (sel) sel.addEventListener('click', confirmar);
    bindGaleria();
  }
  ...
  function confirmar() {
    ...
    Swal.fire({
      ...
      enviarSeleccion(o.id);
    });
  }

  function enviarSeleccion(id) {
    var quitar = pintarEsqueleto('ofertaFicha');
    apiPost('seleccionarOferta', Object.assign(cred(), { id: id, acepto: true }))
      .then(function (r) {
